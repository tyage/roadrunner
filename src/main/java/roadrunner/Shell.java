...
import roadrunner.engine.Engine;
import roadrunner.engine.MDLEvaluator;
import roadrunner.labeller.Labeller;
import roadrunner.marshall.BrowserMarshall;
...
public class Shell {
    ...
    static private Sample[] getSamples() throws IOException, SAXException {
        ...
        for(int h=0; h<samplesURL.length; h++) {
            ...
            try {
                samples[h] = new Sample(samplesURL[h], Config.getPrefs());
            }
            catch (IOException e) {
